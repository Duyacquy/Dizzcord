    ...
    async show(req, res) {
        if(req.isAuthenticated()) {
            ...
            if(allFriend.length === 0) {
                ...
                res.render('help',{noRequestFriend, allFriend: mutipleMongooseToObject(allFriend), user: mongooseToObject(user), userNoti: mutipleMongooseToObject(userNoti), layout: false});
            } else {
                res.render('help', {allFriend: mutipleMongooseToObject(allFriend), user: mongooseToObject(user), userNoti: mutipleMongooseToObject(userNoti), layout: false});
            }
        } else {
            res.redirect('/login');
        }
